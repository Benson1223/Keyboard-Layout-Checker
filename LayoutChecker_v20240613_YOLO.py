...
def convert_and_resize_images(image, output_folder, filename):
    resized_image = cv2.resize(image, (640, 640))
    
    if len(resized_image.shape) == 2:  # Check if the image is grayscale
        resized_image = cv2.cvtColor(resized_image, cv2.COLOR_GRAY2BGR)

    output_image_path = os.path.join(output_folder, filename)
    cv2.imwrite(output_image_path, resized_image)

def analyze_and_save_contours_YOLO(overlay_image, part_name, save_path, cropped_parts_image1, cropped_parts_image2, new_version_folder, old_version_folder):
    image_np = np.array(overlay_image)
    bgr_image = cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR)
    gray_image = cv2.cvtColor(bgr_image, cv2.COLOR_RGB2GRAY)
    ...
    for i, contour in enumerate(contours):
        ...
        if not overlap:
            ...
            if green_area + red_area > 200: #ORI 200
                ...
                test1_path = os.path.join(new_version_folder, f"{part_name}_new_version_{index}.png")
                test2_path = os.path.join(old_version_folder, f"{part_name}_old_version_{index}.png")
                Image.fromarray(cropped_roi_part_image1_rgb).save(test1_path)
                Image.fromarray(cropped_roi_part_image2_rgb).save(test2_path)

                # Resize and save the images to after_resize folder
                new_version_resize_folder = os.path.join(new_version_folder, 'after_resize')
                old_version_resize_folder = os.path.join(old_version_folder, 'after_resize')
                if not os.path.exists(new_version_resize_folder):
                    os.makedirs(new_version_resize_folder)
                if not os.path.exists(old_version_resize_folder):
                    os.makedirs(old_version_resize_folder)
                
                convert_and_resize_images(cropped_roi_part_image1_rgb, new_version_resize_folder, f"{part_name}_new_version_{index}.png")
                convert_and_resize_images(cropped_roi_part_image2_rgb, old_version_resize_folder, f"{part_name}_old_version_{index}.png")
                ...
                # Append the current contour's bounding box to the list of valid contours
                valid_contours.append([x, y, x + w, y + h])
                if not os.listdir(new_version_resize_folder):
                    os.rmdir(new_version_resize_folder)
                if not os.listdir(old_version_resize_folder):
                    os.rmdir(old_version_resize_folder)
...
def perform_yolo_detection(new_version_folder, old_version_folder, compare_result_folder, model_path='best.pt'):
    ...
    columns_width = [40, 25, 65, 25, 65]  
    for i, width in enumerate(columns_width, start=1):
        ws.column_dimensions[get_column_letter(i)].width = width

    column_titles = ["Compare Result", "New Ver Key Image", "New Ver Result", "Old Ver Key Image", "Old Ver Result"]
    for i, title in enumerate(column_titles, start=1):
        ws.cell(row=1, column=i).value = title

    yolo_dirs = [os.path.join(new_version_folder, 'after_resize'), os.path.join(old_version_folder, 'after_resize')]
    save_dirs = [os.path.join(new_version_folder, 'detected_images1'), os.path.join(old_version_folder, 'detected_images2')]
    compare_dir = compare_result_folder
    ...
    print(os.listdir(compare_dir))
    print(os.listdir(yolo_dirs[0]))
    print(os.listdir(yolo_dirs[1]))
    all_files = os.listdir(compare_dir)
    image_files_png = [file for file in all_files if file.endswith('.png')]

    for image_files in zip(os.listdir(yolo_dirs[0]), os.listdir(yolo_dirs[1]), image_files_png):
        compare_image_path = os.path.join(compare_dir, image_files[2])
        if is_image_file(compare_image_path) and os.path.exists(compare_image_path) and os.access(compare_image_path, os.R_OK):
    ...
    excel_path = os.path.join(compare_result_folder, "YOLO_Detection_Results.xlsx")
    wb.save(excel_path)
...
for country, pair in image_pairs.items():
    ...
    ###NEW Generate overlay images for latest design p1 and latest design p2 with YOLO
    if design_latest_path and os.path.exists(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p1.png")) and os.path.exists(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png")):
        latest_design_p1 = cv2.imread(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p1.png"), cv2.IMREAD_GRAYSCALE)
        latest_design_p2 = cv2.imread(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        overlay_images_design_latest_single = Generate_overlay_images_single(latest_design_p1, latest_design_p2)
        ...
        compare_result_folder = os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_design_p1_and_design_p2_error_key")
        new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        
        if not os.path.exists(compare_result_folder):
            os.makedirs(compare_result_folder)

        os.makedirs(new_version_folder, exist_ok=True)
        os.makedirs(old_version_folder, exist_ok=True)

        for idx, overlay_image in enumerate(overlay_images_design_latest_multi):
            part_name = ['upper_left', 'lower_left', 'upper_middle', 'lower_middle', 'upper_right', 'lower_right'][idx]
            overlay_image.save(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_overlay_latest_design_{part_name}.png"))
            analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_latest_design_p1, cropped_parts_latest_design_p2, new_version_folder, old_version_folder)

        
        
        new_version_empty = not any(os.path.isfile(os.path.join(new_version_folder, f)) for f in os.listdir(new_version_folder))
        old_version_empty = not any(os.path.isfile(os.path.join(old_version_folder, f)) for f in os.listdir(old_version_folder))

        if new_version_empty and old_version_empty:
            os.rmdir(new_version_folder)
            os.rmdir(old_version_folder)
        else:
            perform_yolo_detection(new_version_folder, old_version_folder, compare_result_folder)
        
        if not os.listdir(compare_result_folder):
            os.rmdir(compare_result_folder)
    ...
    ###NEW Generate overlay images for second newest design p2 and latest design p2 with YOLO
    if design_second_newest_path and os.path.exists(os.path.join(design_second_newest_path, f"{os.path.basename(design_second_newest_path)}_p2.png")) and design_latest_path and os.path.exists(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png")):
        second_newest_design_p2 = cv2.imread(os.path.join(design_second_newest_path, f"{os.path.basename(design_second_newest_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        latest_design_p2 = cv2.imread(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        overlay_images_design_second_latest_single = Generate_overlay_images_single(second_newest_design_p2, latest_design_p2)
        ...
        compare_result_folder = os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_second_latest_error_key")
        new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        ...
        for idx, overlay_image in enumerate(overlay_images_design_second_latest_multi):
            part_name = ['upper_left', 'lower_left', 'upper_middle', 'lower_middle', 'upper_right', 'lower_right'][idx]
            analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_second_newest_design_p2, cropped_parts_latest_design_p2, new_version_folder, old_version_folder)
    ...
    #     if not os.listdir(save_contours_path):
    #         os.rmdir(save_contours_path)
    # Generate overlay images for reference p2 and latest design p2(with yolo)
    if reference_path and os.path.exists(os.path.join(reference_path, f"{os.path.basename(reference_path)}_p2.png")) and design_latest_path and os.path.exists(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png")):
        reference_p2 = cv2.imread(os.path.join(reference_path, f"{os.path.basename(reference_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        latest_design_p2 = cv2.imread(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        overlay_images_reference_latest_single = Generate_overlay_images_single(reference_p2, latest_design_p2)
        ...
        compare_result_folder = os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_reference_design_error_key")
        new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        ...
        for idx, overlay_image in enumerate(overlay_images_reference_design_multi):
            part_name = ['upper_left', 'lower_left', 'upper_middle', 'lower_middle', 'upper_right', 'lower_right'][idx]
            overlay_image.save(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_overlay_reference_design_{part_name}.png"))
            analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_reference, cropped_parts_design, new_version_folder, old_version_folder)
    ...
    # Generate overlay images for reference p2 and latest PDK p1(with yolo)
    if reference_path and os.path.exists(os.path.join(reference_path, f"{os.path.basename(reference_path)}_p2.png")) and pdk_latest_path and os.path.exists(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_p1.png")):
        reference_p2 = cv2.imread(os.path.join(reference_path, f"{os.path.basename(reference_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        pdk_p1 = cv2.imread(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_p1.png"), cv2.IMREAD_GRAYSCALE)
        overlay_images_reference_pdk_single = Generate_overlay_images_single(reference_p2, pdk_p1)
        ...
        compare_result_folder = os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_reference_pdk_error_key")
        new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        ...
        for idx, overlay_image in enumerate(overlay_images_reference_pdk_multi):
            part_name = ['upper_left', 'lower_left', 'upper_middle', 'lower_middle', 'upper_right', 'lower_right'][idx]
            overlay_image.save(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_overlay_reference_pdk_{part_name}.png"))
            analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_reference, cropped_parts_pdk, new_version_folder, old_version_folder)
    ...
    # Generate overlay images for latest design p2 and latest PDK p1(with yolo)
    if design_latest_path and os.path.exists(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png")) and pdk_latest_path and os.path.exists(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_p1.png")):
        latest_design_p2 = cv2.imread(os.path.join(design_latest_path, f"{os.path.basename(design_latest_path)}_p2.png"), cv2.IMREAD_GRAYSCALE)
        pdk_p1 = cv2.imread(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_p1.png"), cv2.IMREAD_GRAYSCALE)
        overlay_images_latest_pdk_single = Generate_overlay_images_single(latest_design_p2, pdk_p1)
        ...
        compare_result_folder = os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_design_pdk_error_key")
        new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        ...
        for idx, overlay_image in enumerate(overlay_images_design_pdk_multi):
            part_name = ['upper_left', 'lower_left', 'upper_middle', 'lower_middle', 'upper_right', 'lower_right'][idx]
            overlay_image.save(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_overlay_design_pdk_{part_name}.png"))
            analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_latest, cropped_parts_pdk, new_version_folder, old_version_folder)
    ...
    if pdk_second_newest_path and os.path.exists(os.path.join(pdk_second_newest_path, f"{os.path.basename(pdk_second_newest_path)}_p1.png")) and pdk_latest_path and os.path.exists(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_p1.png")):
        ...
        # Save the overlay images
        compare_result_folder = os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_second_latest_error_key")
        new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        if not os.path.exists(compare_result_folder):
            os.makedirs(compare_result_folder)
        ...
        for idx, overlay_image in enumerate(overlay_images_pdk_second_latest_multi):
            part_name = ['upper_left', 'lower_left', 'upper_middle', 'lower_middle', 'upper_right', 'lower_right'][idx]
            overlay_image.save(os.path.join(pdk_latest_path, f"{os.path.basename(pdk_latest_path)}_overlay_second_latest_{part_name}.png"))
            analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_second_newest_pdk_p1, cropped_parts_latest_pdk_p1, new_version_folder, old_version_folder)
...
for country, pair in image_pairs.items():
    if country != 'US':  # Check if the country is not CH or US
        ...
        #             compare_result_folder = os.path.join(fra_latest_path, f"{os.path.basename(fra_latest_path)}_CH_error_key")
        #             new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
        #             old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
        #             if not os.path.exists(compare_result_folder):
        #                 os.makedirs(compare_result_folder)
        ...
        #                 analyze_and_save_contours_YOLO(overlay_image, part_name, compare_result_folder, cropped_parts_fra_p2, cropped_parts_c_p2, new_version_folder, old_version_folder)
        ...
        if 'US' in image_pairs:
            ...
            if c_latest_path:
                ...
                if os.path.exists(fra_p2_path) and os.path.exists(c_p2_path):
                    ...
                    # Save the overlay images and analyze contours
                    compare_result_folder = os.path.join(fra_latest_path, f"{os.path.basename(fra_latest_path)}_US_error_key")
                    new_version_folder = os.path.join(compare_result_folder, 'new_used_image')
                    old_version_folder = os.path.join(compare_result_folder, 'old_used_image')
